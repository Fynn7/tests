...
class SchedulerNode(Node):
    ...
    def hlc_action_result_callback(self, goal_handle):
        ...
        # feedback loop
        # while requested_processstep not in {3, 5, 6, 20, 21, 22, 23}: # Endzustände
        if requested_processstep < 3:
            # requested_processstep
            # self.get_logger().info('Finished Processstep and publish it 1 : "%d".' % self.processstep)
            ...
            if self.succeeded:
                ...
                msg_processtep.processstep = requested_processstep
                ...
                self.get_logger().info('Finished Processstep and publish it 2 : "%d".' %
                                       self.processstep)
                ...
                self.get_logger().info('Finished Processstep and publish it 4 : "%d".' %
                                       self.processstep)
                requested_processstep += 1

        elif requested_processstep == 4:

            self.get_logger().info('Finished Processstep and publish it 5 : "%d".' %
                                   self.processstep)
            # requested_processstep
            # self.get_logger().info('Finished Processstep and publish it 1 : "%d".' % self.processstep)
            ...
            self.get_logger().info('Finished Processstep and publish it 6 : "%d".' %
                                   self.processstep)

        elif requested_processstep == 7:
            ...
            if self.succeeded:
                ...
                self.get_logger().info('Finished Processstep and publish it 4 : "%d".' %
                                       self.processstep)
                requested_processstep += 1
            self.get_logger().info('Finished Processstep and publish it 7 : "%d".' %
                                   self.processstep)

        elif 21 <= requested_processstep <= 23:
            ...
            if self.succeeded:
                ...
                self.get_logger().info('Finished Processstep and publish it 4 : "%d".' %
                                       self.processstep)
                requested_processstep += 1
            self.get_logger().info('Finished Processstep and publish it 8 : "%d".' %
                                   self.processstep)

        else:
            self.get_logger().info('Finished Processstep and publish it 11 : "%d".' %
                                   self.processstep)
    ...
    def processstepsuccess_callback(self, msg: ProcessStep):
        ...
        if (self.processstep == self.processstepsuccess_gui == self.processstepsuccess_rpi == self.processstepsuccess_scheduler) and (self.processstep not in self.processstep_manualconfirm):
            ...
            self.get_logger().info('All Nodes finished. Automatically starting Processstep "%s".' %
                                   msg_processtep.processstep)
            # self.gui_rpi_scheduler_ready=True
