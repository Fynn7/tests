...
class SchedulerNode(Node):
    ...
    def hlc_action_result_callback(self, goal_handle):
        ...
        while self.processstep not in {4, 5, 6, 20, 21, 22, 23}:
            time.sleep(2)
    ...
    def rpiinputs_callback(self, msg: RPiInputs):
        ...
        if self.confirm:
            ...
            # if (self.processstep == self.processstepsuccess_gui == self.processstepsuccess_rpi == self.processstepsuccess_rpi_handgriff == self.processstepsuccess_scheduler) and (self.processstep in self.processstep_manualconfirm):
                ...
                self.get_logger().info(f"ps: {self.processstep} gui{self.processstepsuccess_gui} rpi:{self.processstepsuccess_rpi} scheduelr{self.processstepsuccess_scheduler} handgriff{self.processstepsuccess_rpi_handgriff}manualconfirm{self.processstep_manualconfirm}")
