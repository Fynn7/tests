...
from hmi_interfaces.msg import BoolStamped
...
class SchedulerNode(Node):
    ...
    def __init__(self):
        ...
        self.isAlive_robotcontroller_publisher_ = self.create_publisher(
            BoolStamped, 'isAlive_robotcontroller', 10)

    def publish_isAlive_robotcontroller(self, isAlive: bool):
        msg = BoolStamped()
        msg.data = isAlive
        msg.stamp = self.get_clock().now().to_msg()
        self.isAlive_robotcontroller_publisher_.publish(msg)
    ...
    def hlc_action_result_callback(self, goal_handle):
        ...
        first_step = FP_MAPPER[goal_handle.request.function_id]
        self.processstep = first_step
        ...
        def exec_processstep():
            '''
            The whole process for each processstep
            '''
            time.sleep(2)
        ...
        if first_step == 0:
            ...
            for _ in range(14):
                exec_processstep()
        elif first_step == 6 or first_step == 21 or first_step == 22 or first_step == 23:
            exec_processstep()
        else:
            self.get_logger().info("Fehler: Nicht ein valides Processstep/function_id!")
    ...
    def rpiinputs_callback(self, msg: RPiInputs):
        ...
        if self.confirm:
            ...
            if (self.processstep == self.processstepsuccess_gui == self.processstepsuccess_rpi == self.processstepsuccess_scheduler) and (self.processstep in self.processstep_manualconfirm):
                ...
                self.get_logger().info(
                    f"ps: {self.processstep} gui{self.processstepsuccess_gui} rpi:{self.processstepsuccess_rpi} scheduelr{self.processstepsuccess_scheduler} handgriff{self.processstepsuccess_rpi_handgriff}manualconfirm{self.processstep_manualconfirm}")
